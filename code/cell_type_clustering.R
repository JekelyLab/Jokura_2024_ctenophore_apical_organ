...
tag_stats_per_skid <- data.frame(matrix(ncol = length(characters)+1, nrow = 0))
colnames(tag_stats_per_skid) <- c("skid", characters)
tag_stats <- function(skid) {
  ...
  counts <- mapply(tag_count, list(tags), characters)
...
for (skid in skids) {
  ...
  if (!is.null(tag_counts)) {
    tag_stats_per_skid[nrow(tag_stats_per_skid) + 1,] = c(skid, tag_counts)
  }
}

tag_stats_noskid <- tag_stats_per_skid[,-1]
